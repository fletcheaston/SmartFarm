...
void SmartFarmMeasure::setupAll(String boardID) {
  Serial.begin(57600);
  setupSD();
  delay(1000);
  setupWM();
...
//DS18B20 temperature
void SmartFarmMeasure::setupTemps() {//temp setup function
...
/*void SmartFarmMeasure::setupDecSensors() {
  ...
  //The first time it is called is within the setTaken function @ '0'
  //within the setTaken function, the addressRegister gets a true or false depending on...IDK
  ...
  //I don't believe its the port, becuase of board v5.4
...
//Scanning Decagon 5TE addresses
  ...
  if (!found) {
    Text += "No DECAGON 5TE sensors found.";
  } // stop here

  Text += " Found ";
  Text += String(found, DEC);
  Text += " Decagon 5TE sensors.";
  return Text;
}
...
*/
//New readWM gets rid of high values and takes in watermark position
String SmartFarmMeasure::readWM(String boardID, int WMpos1, int WMpos2, int WMpos3) {
  // Scan watermark connections
  int WMPin1 = 4;
  int WMPin2 = 5;
  String WMdata = boardID;
  ...
  for (int i = 0; i < 3; i++) // Go through each port to read data
  {
    ...
    for (int j = 0; j < 5; j++) {
      ...
      if (R > 0.0) //limit of resistance, sometimes a reading of +100k will be considered a port active when no sensor is connected
      {
        RArray[j] = R;
      }
      else {
...
//Decagon
/*void SmartFarmMeasure::readDecSensors(String boardID, int numDecSens) {
...
// This returns the temp sensor as a string
/*String SmartFarmMeasure::readTemps(String boardID) {//deprecated Auto temp scan 2/12/18 readTemp_West & East
  String NA = "NA";
  String results = boardID + ' ';
  float tempC;
  char fbuff[6];
  ...
  for (i = 0; i < numberOfDevices; i++) { //sensors exist
  ...
  tempC = sensors.getTempC(therms[i]);
  // results += "Temp C: ";
  //result = String(tempC, 2) + ' ';
  dtostrf(tempC, 5, 2, fbuff); //dtostrf(FLOAT,WIDTH,PRECSISION,BUFFER)
  //WIDTH = PRECSISION+2
  //PRECSISION = the number of characters after the decimal point
  //BUFFER = storage of the char string
  results += fbuff;
  results += ' ';
  }
...
/*
  ...
  void SmartFarmMeasure::setupTemps() {//deprecated 2/12/18 for auto scan sensors
