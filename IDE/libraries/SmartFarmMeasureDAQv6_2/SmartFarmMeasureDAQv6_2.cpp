...
void SmartFarmMeasure::printBufferToScreen(char i)
{
	// prints the data 1.22 0.00 26.5
	//never gets called
}
...
String SmartFarmMeasure::readWM(String boardID)
{
	...
	byte initCount = 0;
	byte maxCount = 5;
	String WMdata = boardID + " SM123456";

	for (byte i = initCount;i <= maxCount; i++)
	{
		String Rstring = "";
		float RArray[5];
		float Rs;
		...
		selectMuxPin(i);
		for (int j = 0;j < 5; j++) //take 5 measurements
		{
...
String SmartFarmMeasure::readWM(String boardID, int WMpos1, int WMpos2, int WMpos3) {
  ...
  for (byte i = initCount; i <= maxCount; i++) // Go through ports 1,2,3 to read data
  {
    String Rstring = "";
    float RArray[5];
    float Rs;
